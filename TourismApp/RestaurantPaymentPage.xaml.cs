using System.Net.Http.Json;
using System.Text.Json;

namespace TourismApp;

[QueryProperty(nameof(RestaurantIdStr), "restaurantId")]
[QueryProperty(nameof(RestaurantNameStr), "restaurantName")]
[QueryProperty(nameof(PlanTypeStr), "planType")]
[QueryProperty(nameof(AmountStr), "amount")]
public partial class RestaurantPaymentPage : ContentPage
{
    private readonly HttpClient _httpClient;
    private int _restaurantId;
    private string _restaurantName = "";
    private string _planType = "Normal";
    private decimal _amount;
    private int _paymentId;
    private string _referenceCode = "";
    private CancellationTokenSource? _pollCts;
    private bool _paymentConfirmed;

    private const string BankId = "970423";
    private const string AccountNo = "24619072005";
    private const string AccountName = "MAC ANH HAO";
    private const string BankDisplayName = "TPBank";

    public string RestaurantIdStr
    {
        set { if (int.TryParse(value, out var id)) _restaurantId = id; }
    }

    public string RestaurantNameStr
    {
        set { _restaurantName = Uri.UnescapeDataString(value ?? ""); }
    }

    public string PlanTypeStr
    {
        set { _planType = value ?? "Normal"; }
    }

    public string AmountStr
    {
        set { if (decimal.TryParse(value, out var a)) _amount = a; }
    }

    public RestaurantPaymentPage(HttpClient httpClient)
    {
        InitializeComponent();
        _httpClient = httpClient;
    }

    protected override async void OnAppearing()
    {
        base.OnAppearing();

        lblRestaurantName.Text = _restaurantName;
        lblPlanType.Text = _planType == "Premium" ? "⭐ Premium (1 năm)" : "🏪 Thường";
        lblPrice.Text = $"{_amount:N0}đ";

        lblBankName.Text = $"Ngân hàng: {BankDisplayName}";
        lblAccountNo.Text = $"STK: {AccountNo}";
        lblAccountName.Text = $"Chủ TK: {AccountName}";

        await CreatePaymentAndShowQR();
    }

    protected override async void OnDisappearing()
    {
        base.OnDisappearing();
        _pollCts?.Cancel();

        if (!_paymentConfirmed && _restaurantId > 0)
        {
            try
            {
                var token = await SecureStorage.GetAsync("auth_token");
                if (!string.IsNullOrEmpty(token))
                {
                    var request = new HttpRequestMessage(HttpMethod.Delete, $"api/payments/cancel-restaurant/{_restaurantId}");
                    request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                    await _httpClient.SendAsync(request);
                }
            }
            catch { }
        }
    }

    private async Task CreatePaymentAndShowQR()
    {
        var token = await SecureStorage.GetAsync("auth_token");
        if (string.IsNullOrEmpty(token))
        {
            await DisplayAlert("Lỗi", "Bạn cần đăng nhập để thanh toán", "OK");
            return;
        }

        try
        {
            lblStatus.Text = "⏳ Đang tạo yêu cầu thanh toán...";

            var request = new HttpRequestMessage(HttpMethod.Post, "api/payments/register-restaurant");
            request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
            request.Content = JsonContent.Create(new
            {
                restaurantId = _restaurantId,
                amount = _amount,
                paymentMethod = "QR",
                planType = _planType
            });

            var response = await _httpClient.SendAsync(request);
            var json = await response.Content.ReadAsStringAsync();

            if (!response.IsSuccessStatusCode)
            {
                lblStatus.Text = $"❌ {json}";
                return;
            }

            using var doc = JsonDocument.Parse(json);
            var root = doc.RootElement;

            _paymentId = root.GetProperty("paymentId").GetInt32();
            _referenceCode = root.TryGetProperty("referenceCode", out var refProp)
                ? refProp.GetString() ?? $"TPA{_paymentId}"
                : $"TPA{_paymentId}";
            var amount = root.TryGetProperty("amount", out var amtProp)
                ? amtProp.GetDecimal() : _amount;

            var qrUrl = $"https://img.vietqr.io/image/{BankId}-{AccountNo}-compact2.png"
                + $"?amount={amount:0}"
                + $"&addInfo={Uri.EscapeDataString(_referenceCode)}"
                + $"&accountName={Uri.EscapeDataString(AccountName)}";

            imgQRCode.Source = ImageSource.FromUri(new Uri(qrUrl));

            lblAmount.Text = $"Số tiền: {amount:N0}đ";
            lblTransferContent.Text = $"Nội dung CK: {_referenceCode}";
            lblStatus.Text = "";

            framePolling.IsVisible = true;
            StartPolling(token);
        }
        catch (Exception ex)
        {
            lblStatus.Text = $"❌ Lỗi: {ex.Message}";
        }
    }

    private void StartPolling(string token)
    {
        _pollCts?.Cancel();
        _pollCts = new CancellationTokenSource();
        var ct = _pollCts.Token;

        _ = Task.Run(async () =>
        {
            while (!ct.IsCancellationRequested)
            {
                await Task.Delay(3000, ct);
                if (ct.IsCancellationRequested) break;

                try
                {
                    var request = new HttpRequestMessage(HttpMethod.Get, $"api/payments/check-restaurant/{_restaurantId}");
                    request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                    var response = await _httpClient.SendAsync(request, ct);

                    if (response.IsSuccessStatusCode)
                    {
                        var json = await response.Content.ReadAsStringAsync(ct);
                        using var doc = JsonDocument.Parse(json);
                        var isPaid = doc.RootElement.GetProperty("isPaid").GetBoolean();

                        if (isPaid)
                        {
                            MainThread.BeginInvokeOnMainThread(async () =>
                            {
                                _paymentConfirmed = true;
                                _pollCts?.Cancel();

                                framePolling.IsVisible = false;
                                frameSuccess.IsVisible = true;

                                await DisplayAlert("🎉 Thành công!",
                                    $"Đăng ký nhà hàng \"{_restaurantName}\" thành công!\nNhà hàng đã được kích hoạt.",
                                    "Bắt đầu quản lý");

                                await Shell.Current.GoToAsync("../..");
                            });
                            break;
                        }
                    }
                }
                catch (OperationCanceledException) { break; }
                catch { }
            }
        }, ct);
    }
}
